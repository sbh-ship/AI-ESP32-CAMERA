...
void setup() {
  ...
  if (config.pixel_format == PIXFORMAT_JPEG) {
    s->set_framesize(s, psramFound() ? FRAMESIZE_HD : FRAMESIZE_VGA);
...
// Setup LED FLash if LED pin is defined in camera_pins.h
  ...
  Serial.print("SSID: ");
  Serial.println(ssid);
  Serial.print("Camera Ready! Control panel: http://");
...
void loop() {
  ...
  if (millis() - lastStatusLog >= 5000) {
    ...
    Serial.printf(
      "WiFi status=%d RSSI=%d SSID=%s IP=%s Host=%s\n",
      WiFi.status(),
